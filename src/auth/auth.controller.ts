...
export class AuthController {
  ...
  async login(@Res() response: Response): Promise<any> {
    ...
    const userID = 'dummyuser'; // if userID was userDiD we'd need a DiD resolution method
  ...
  async devices(): Promise<any> {
    const events = [
      { date: new Date().getUTCDate(), description: 'Super event 1' },
      { date: new Date().getUTCDate(), description: 'Super event 2' },
      { date: new Date().getUTCDate(), description: 'Super event 3' },
      { date: new Date().getUTCDate(), description: 'Super event 4' },
    ];
    return events;
  }
